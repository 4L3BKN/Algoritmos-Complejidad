...
int main() {
    ...
    // Verificar si se leyeron correctamente dos matrices
    if (matrices.size() < 2) {
        cerr << "No se pudieron leer dos matrices del archivo." << endl;
        return 1;
    }

    // Obtener matrices A y B
    vector<vector<int>> matrix_A = matrices[0];
    vector<vector<int>> matrix_B = matrices[1];

    // Verificar que ambas matrices sean cuadradas y de igual tamaño
    if (matrix_A.size() != matrix_A[0].size() || matrix_B.size() != matrix_B[0].size() || matrix_A.size() != matrix_B.size()) {
        cerr << "Las matrices deben ser cuadradas y de igual tamaño." << endl;
        return 1;
    }

    //print("Array A", matrix_A, 0, 0, matrix_A.size() - 1, matrix_A[0].size() - 1);
    //print("Array B", matrix_B, 0, 0, matrix_B.size() - 1, matrix_B[0].size() - 1);
    ...
    //print("Result Array", result_matrix, 0, 0, result_matrix.size() - 1, result_matrix[0].size() - 1);
